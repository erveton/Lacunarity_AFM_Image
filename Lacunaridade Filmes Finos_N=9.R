...
#Passo 6: cobrir toda a matriz binária com as caixas de tamanho N e Contar o número de pixels lacunaridade em cada caixa.

vnum0 = c(NULL)
for (n in c(1:248)) {
  
  i<-c(1+(n-1),2+(n-1),3+(n-1),4+(n-1),5+(n-1),6+(n-1),7+(n-1),8+(n-1),9+(n-1))
  
  for (k in c(2:249)){
    
    j<-c(k-1,k,k+1,k+2,k+3,k+4,k+5,k+6,k+7)
